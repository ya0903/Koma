...
class ServerRepositoryImpl @Inject constructor(
    ...
    override suspend fun getActive(): Server? {
        ...
        // Ensure Coil auth cache is warm for the active server
        val host = runCatching { URL(entity.baseUrl).host }.getOrDefault("")
        if (host.isNotEmpty()) {
            coilAuthInterceptor.setActiveServer(entity.id, host)
        }
