...
class KomgaMediaServer(
    ...
    override suspend fun onDeckBooks(): List<Book> {
        return api.getOnDeck(size = 20).content.map { it.toDomain(id, baseUrl) }
    }

    override suspend fun recentlyAddedSeries(): List<Series> {
        return api.getNewSeries(size = 20).content.map { it.toDomain(id, baseUrl) }
    }

    override suspend fun recentlyUpdatedSeries(): List<Series> {
        return api.getSeries(sort = "lastModified,desc", size = 20).content.map { it.toDomain(id, baseUrl) }
    }

    override suspend fun recentlyAddedBooks(): List<Book> {
        return api.getBooks(sort = "created,desc", size = 20).content.map { it.toDomain(id, baseUrl) }
    }

    override suspend fun recentlyReleasedBooks(): List<Book> {
        return api.getBooks(sort = "metadata.releaseDate,desc", size = 20).content.map { it.toDomain(id, baseUrl) }
    }

    override suspend fun recentlyReadBooks(): List<Book> {
        return api.getBooks(readStatus = "READ", sort = "readProgress.readDate,desc", size = 20).content.map { it.toDomain(id, baseUrl) }
    }
