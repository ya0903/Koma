...
class KomgaMediaServer(
    ...
    override suspend fun search(query: String, filter: SearchFilter): SearchResults {
        val series = api.getSeries(search = query, page = 0, size = 50).content
            .map { it.toDomain(id, baseUrl) }
        val books = api.getBooks(sort = "metadata.titleSort,asc", size = 50).content
            .filter { it.name.contains(query, ignoreCase = true) || it.metadata.title.contains(query, ignoreCase = true) }
            .map { it.toDomain(id, baseUrl) }
